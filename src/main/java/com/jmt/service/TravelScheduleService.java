...
public class TravelScheduleService {
    ...
    //마이페이지에 나의 일정 조회쿼리
    public List<TravelScheduleDto> myTtravelScheduleSelect(String userId){

        String userId1 = userId;
        List<TravelScheduleEntity> select = travelScheduleRepository.myTtravelScheduleSelect(userId,userId1);
    ...
    public List<TravelScheduleDto> travelScheduleSelect(String userId){
        ...
        List<TravelScheduleEntity> select = travelScheduleRepository.travelScheduleSelect(userId);
