...
public class EmitterService {
    ...
    public SseEmitter subscribe(String userid){
        ...
        if(emitter == null) {
            emitter = createEmitter(userid);
        }
        //생성 후, 유저에게 이미터 생성 알림
        List<Notification> notifications =  notificationService.showNotification(userid);
        notifications.forEach(notify->{
            log.debug("notifyChk" + notify);
            if(notify != null) {
                //ToDo : 보낼때 Notification형태로 보내지지 않음 혹은 프론트에서 받지 못함 . 고쳐야함
                sendToClient(userid, notify);
            }else{
                log.debug("notifyChk" + notify);
...
//        log.debug("만들어진 emitter 확인  : " + emitter);
    ...
    */
     ...
     */
    private void sendToClient(String userid, Notification data) {
